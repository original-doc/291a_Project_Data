#!/usr/bin/env python3
"""
FAISS Retrieval Evaluator

Evaluates FAISS retrieval results against ground truth from final_requests.json.
...
The script expects:
    - faiss_result/retrieval_results.json (retrieval results)
...
class RetrievalEvaluator:
    ...
    def __init__(self, results_path: Path, ground_truth_path: Path):
        ...
        """
        print("="*70)
        print("FAISS Retrieval Evaluator")
        print("="*70)
    ...
    def _match_documents(self, retrieved_doc: Dict, relevant_doc: Dict) -> bool:
        ...
        # Match by path only (for docs without specific index)
        if ret_path_norm == rel_path_norm and not rel_func:
            return True
...
def main():
    ...
    output_path = script_dir / "faiss_result" / "evaluation_metrics.json"
    
    # Check if files exist
    if not results_path.exists():
        print(f"❌ Retrieval results not found: {results_path}")
        print("Please run faiss_retrieval.py first to generate results.")
        return
