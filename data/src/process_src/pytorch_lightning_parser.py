...
class PyTorchLightningParser:
    ...
    def extract_functions_from_node(self, node, source_code: bytes, file_path: Path, 
        ...
        if node.type == 'function_definition':
            ...
            if func_name:
                ...
                # Skip functions without docstrings if required
                # (You can make this configurable)
                if docstring and len(docstring) > 10:  # Minimum docstring length
    ...
    def filter_quality(self, min_docstring_length=20, max_complexity=15, 
        ...
        for func in self.categories['core']:
            # Quality checks
            if len(func.docstring) < min_docstring_length:
                continue
...
def main():
    ...
    # Initialize parser
    pl_parser = PyTorchLightningParser(args.repo_path, args.output_dir)
