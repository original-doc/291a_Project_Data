...
class AdvancedPyTorchLightningParser:
    ...
    def create_augmented_scenarios(self, func_node, source_code: bytes, 
        ...
        ACS 0: Short docstring description â†’ Code (default CodeSearchNet)
        ACS 1: Code comments â†’ Code
        ACS 2: Code comments + entire docstring â†’ Code
        ACS 3: Code comments + entire docstring + commit message â†’ Code + comments
        ACS 4: Code comments + entire docstring â†’ Code + code comments
        ACS 5: Short docstring description â†’ Code (simplified)
        """
        pairs = []
        ...
        # ACS 0: Default CodeSearchNet format (short description â†’ code with comments)
        if summary:
        ...
        # ACS 1: Developer comments â†’ code without comments
        if all_comments:
            pairs.append(AugmentedPair(
        ...
        # ACS 2: Full docstring + comments â†’ code without comments
        ...
        # ACS 4: Full docstring + comments â†’ full code with comments
        if docstring:
        ...
        # ACS 5: Short description â†’ code without comments (simplified)
        if summary:
            pairs.append(AugmentedPair(
                nl_text=summary,
                pl_text=code_without_comments,
        ...
        try:
            ...
            def extract_functions(node, class_name=None):
                if node.type == 'function_definition':
                    ...
                    # Include if has docstring OR is important method OR allow all functions
                    if has_docstring or is_important or True:  # Changed to allow all
